...
void snap_io::read_ic(const char *fname)
{
  ...
  /* save the total particle number in Sp->TotNumICPart */
  Sp->TotNumICPart = Sp->TotNumPart;
  ...
  /* output some info about the snapshot for checking */
  mpi_printf("total number of particles in the IC file = %lld\n", Sp->TotNumICPart);
/*
  mpi_printf("IC particle data check\n");
  ...
  for(int i=Sp->TotNumICPart - 10; i<Sp->TotNumICPart-1; i++)
    {
      Sp->print_particle_info(i);
    }
*/
