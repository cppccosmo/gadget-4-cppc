...
#define POWERSPEC_FOLDFAC 16 /* folding factor to obtain an estimate of the power spectrum on very small scales */
  ...
  char theta_spec_fname[MAXLEN_PATH_EXTRA];
  char neutrino_delta_fname[MAXLEN_PATH_EXTRA];
  char y_nu_fname[MAXLEN_PATH_EXTRA];
  char neutrino_delta_stream_fname[MAXLEN_PATH_EXTRA];
  char neutrino_theta_stream_fname[MAXLEN_PATH_EXTRA];
  char delta_grid_fname[MAXLEN_PATH_EXTRA];
  char gaussian_field_snap_fname[MAXLEN_PATH_EXTRA];
...
#ifdef CB_VELDIV
  fft_real *velgridx, *velgridy, *velgridz, *workspacex, *workspacey, *workspacez;  // declare 3 additional fft grids for velocity components for calculating theta
#endif
...
#ifdef CB_VELDIV
  fft_complex *fft_of_velgridx, *fft_of_velgridy, *fft_of_velgridz;
#endif
 ...
 private:
  ...
  void pmforce_uniform_optimized_prepare_velocity(int mode, int *typelist);
 ...
 public:
  ...
  {
    return pow(a / ((All.Omega0 + All.OmegaNuPart + All.OmegaNuLin) + (1 - All.Omega0 - All.OmegaNuPart - All.OmegaNuLin - All.OmegaLambda) * a + All.OmegaLambda * a * a * a), 1.5);
  }
