...
/*
 *  A copy of pmforce_uniform_optimized_prepare_density, but modified to discretise a velocity field for theta calculation.
 */
#ifdef CB_VELDIV
void pm_periodic::pmforce_uniform_optimized_prepare_velocity(int mode, int *typelist)
{
 ...
 */
  ...
  /* allocate cleared velocity fields */
  velgridx = (fft_real *)Mem.mymalloc_movable_clear(&velgridx, "velgridx", maxfftsize * sizeof(fft_real));
  velgridy = (fft_real *)Mem.mymalloc_movable_clear(&velgridy, "velgridy", maxfftsize * sizeof(fft_real));
  velgridz = (fft_real *)Mem.mymalloc_movable_clear(&velgridz, "velgridz", maxfftsize * sizeof(fft_real));
  ...
  for(size_t i = 0; i < nimport; i++)
    {
      ...
      double vx = partin[i].Vel[0];
      double vy = partin[i].Vel[1];
      double vz = partin[i].Vel[2];
      ...
      if(myplan.slab_to_task[slab_x] == ThisTask)
        {
          ...
          velgridx[FI(slab_x, slab_y, slab_z)] += (vx * (1.0 - dx) * (1.0 - dy) * (1.0 - dz));
          velgridy[FI(slab_x, slab_y, slab_z)] += (vy * (1.0 - dx) * (1.0 - dy) * (1.0 - dz));
          velgridz[FI(slab_x, slab_y, slab_z)] += (vz * (1.0 - dx) * (1.0 - dy) * (1.0 - dz));

          velgridx[FI(slab_x, slab_y, slab_zz)] += (vx * (1.0 - dx) * (1.0 - dy) * (dz));
          velgridy[FI(slab_x, slab_y, slab_zz)] += (vy * (1.0 - dx) * (1.0 - dy) * (dz));
          velgridz[FI(slab_x, slab_y, slab_zz)] += (vz * (1.0 - dx) * (1.0 - dy) * (dz));

          velgridx[FI(slab_x, slab_yy, slab_z)] += (vx * (1.0 - dx) * (dy) * (1.0 - dz));
          velgridy[FI(slab_x, slab_yy, slab_z)] += (vy * (1.0 - dx) * (dy) * (1.0 - dz));
          velgridz[FI(slab_x, slab_yy, slab_z)] += (vz * (1.0 - dx) * (dy) * (1.0 - dz));

          velgridx[FI(slab_x, slab_yy, slab_zz)] += (vx * (1.0 - dx) * (dy) * (dz));
          velgridy[FI(slab_x, slab_yy, slab_zz)] += (vy * (1.0 - dx) * (dy) * (dz));
          velgridz[FI(slab_x, slab_yy, slab_zz)] += (vz * (1.0 - dx) * (dy) * (dz));
        }
      ...
      if(myplan.slab_to_task[slab_xx] == ThisTask)
        {
          ...
          velgridx[FI(slab_xx, slab_y, slab_z)] += (vx * (dx) * (1.0 - dy) * (1.0 - dz));
          velgridy[FI(slab_xx, slab_y, slab_z)] += (vy * (dx) * (1.0 - dy) * (1.0 - dz));
          velgridz[FI(slab_xx, slab_y, slab_z)] += (vz * (dx) * (1.0 - dy) * (1.0 - dz));

          velgridx[FI(slab_xx, slab_y, slab_zz)] += (vx * (dx) * (1.0 - dy) * (dz));
          velgridy[FI(slab_xx, slab_y, slab_zz)] += (vy * (dx) * (1.0 - dy) * (dz));
          velgridz[FI(slab_xx, slab_y, slab_zz)] += (vz * (dx) * (1.0 - dy) * (dz));

          velgridx[FI(slab_xx, slab_yy, slab_z)] += (vx * (dx) * (dy) * (1.0 - dz));
          velgridy[FI(slab_xx, slab_yy, slab_z)] += (vy * (dx) * (dy) * (1.0 - dz));
          velgridz[FI(slab_xx, slab_yy, slab_z)] += (vz * (dx) * (dy) * (1.0 - dz));

          velgridx[FI(slab_xx, slab_yy, slab_zz)] += (vx * (dx) * (dy) * (dz));
          velgridy[FI(slab_xx, slab_yy, slab_zz)] += (vy * (dx) * (dy) * (dz));
          velgridz[FI(slab_xx, slab_yy, slab_zz)] += (vz * (dx) * (dy) * (dz));
        }
    }
      ...
      if(col0 >= first_col && col0 <= last_col)
        {
          ...
          velgridx[FCxy(col0, slab_z)] += (vx * (1.0 - dx) * (1.0 - dy) * (1.0 - dz));
          velgridy[FCxy(col0, slab_z)] += (vy * (1.0 - dx) * (1.0 - dy) * (1.0 - dz));
          velgridz[FCxy(col0, slab_z)] += (vz * (1.0 - dx) * (1.0 - dy) * (1.0 - dz));

          velgridx[FCxy(col0, slab_zz)] += (vx * (1.0 - dx) * (1.0 - dy) * (dz));
          velgridy[FCxy(col0, slab_zz)] += (vy * (1.0 - dx) * (1.0 - dy) * (dz));
          velgridz[FCxy(col0, slab_zz)] += (vz * (1.0 - dx) * (1.0 - dy) * (dz));
        }
      ...
      if(col1 >= first_col && col1 <= last_col)
        {
          ...
          velgridx[FCxy(col1, slab_z)] += (vx * (1.0 - dx) * (dy) * (1.0 - dz));
          velgridy[FCxy(col1, slab_z)] += (vy * (1.0 - dx) * (dy) * (1.0 - dz));
          velgridz[FCxy(col1, slab_z)] += (vz * (1.0 - dx) * (dy) * (1.0 - dz));

          velgridx[FCxy(col1, slab_zz)] += (vx * (1.0 - dx) * (dy) * (dz));
          velgridy[FCxy(col1, slab_zz)] += (vy * (1.0 - dx) * (dy) * (dz));
          velgridz[FCxy(col1, slab_zz)] += (vz * (1.0 - dx) * (dy) * (dz));
        }
      ...
      if(col2 >= first_col && col2 <= last_col)
        {
          ...
          velgridx[FCxy(col2, slab_z)] += (vx * (dx) * (1.0 - dy) * (1.0 - dz));
          velgridy[FCxy(col2, slab_z)] += (vy * (dx) * (1.0 - dy) * (1.0 - dz));
          velgridz[FCxy(col2, slab_z)] += (vz * (dx) * (1.0 - dy) * (1.0 - dz));

          velgridx[FCxy(col2, slab_zz)] += (vx * (dx) * (1.0 - dy) * (dz));
          velgridy[FCxy(col2, slab_zz)] += (vy * (dx) * (1.0 - dy) * (dz));
          velgridz[FCxy(col2, slab_zz)] += (vz * (dx) * (1.0 - dy) * (dz));
        }
      ...
      if(col3 >= first_col && col3 <= last_col)
        {
          ...
          velgridx[FCxy(col3, slab_z)] += (vx * (dx) * (dy) * (1.0 - dz));
          velgridy[FCxy(col3, slab_z)] += (vy * (dx) * (dy) * (1.0 - dz));
          velgridz[FCxy(col3, slab_z)] += (vz * (dx) * (dy) * (1.0 - dz));

          velgridx[FCxy(col3, slab_zz)] += (vx * (dx) * (dy) * (dz));
          velgridy[FCxy(col3, slab_zz)] += (vy * (dx) * (dy) * (dz));
          velgridz[FCxy(col3, slab_zz)] += (vz * (dx) * (dy) * (dz));
        }
    }
...
#ifdef CB_VELDIV
      double vx = sqrt(All.Time) * partin[i].Vel[0];
      double vy = sqrt(All.Time) * partin[i].Vel[1];
      double vz = sqrt(All.Time) * partin[i].Vel[2];
#endif
...
void pm_periodic::pmforce_periodic(int mode, int *typelist)
{
...
#ifdef CB_VELDIV
  workspacex = (fft_real *)Mem.mymalloc_movable(&workspacex, "workspacex", maxfftsize * sizeof(fft_real));
  workspacey = (fft_real *)Mem.mymalloc_movable(&workspacey, "workspacey", maxfftsize * sizeof(fft_real));
  workspacez = (fft_real *)Mem.mymalloc_movable(&workspacez, "workspacez", maxfftsize * sizeof(fft_real));
#endif  
...
#ifdef CB_VELDIV
  fft_of_velgridx = (fft_complex *)&velgridx[0];
  fft_of_velgridy = (fft_complex *)&velgridy[0];
  fft_of_velgridz = (fft_complex *)&velgridz[0];
#endif
  ...
  /* Do the FFT of the velocity fields */
#ifdef CB_VELDIV
#ifndef FFT_COLUMN_BASED
  my_slab_based_fft(&myplan, &velgridx[0], &workspacex[0], 1);
  my_slab_based_fft(&myplan, &velgridy[0], &workspacey[0], 1);
  my_slab_based_fft(&myplan, &velgridz[0], &workspacez[0], 1);
#else
  my_column_based_fft(&myplan, velgridx, workspacex, 1);
  my_column_based_fft(&myplan, velgridy, workspacey, 1);
  my_column_based_fft(&myplan, velgridz, workspacez, 1);
#endif
#endif
...
//#ifndef ADDITIONAL_GRID
  ...
  /* free stuff */
#ifdef CB_VELDIV
  Mem.myfree(workspacez);
  Mem.myfree(workspacey);
  Mem.myfree(workspacex);
#endif
...
void pm_periodic::pmforce_measure_powerspec_inline(int flag, int *typeflag, double *k_out, double *power_out)
{
  ...
  for(int i = 0; i < Sp->NumPart; i++) {
    //if(P[i].getType() == 1) // we only want the CDM power spectrum in this case
      //{ 
        mass += Sp->P[i].getMass(); 
...
void pm_periodic::pmforce_measure_powerspec(int flag, int *typeflag)
{
...
#ifdef CB_VELDIV
  double SumThetaUncorrected[BINS_PS];
  double ThetaUncorrected[BINS_PS];

  double vtotx = 0, vtoty = 0, vtotz = 0;
#endif
...
#ifdef CB_VELDIV
  double facvx = 1.0 / vtotx;
  double facvy = 1.0 / vtoty;
  double facvz = 1.0 / vtotz;
#endif

  /* prepare an additional array for the full 3D delta */
  /* each processor is responsible for their own local patch of the rhogrid, the mpi rank of each process should be indicative of the order of patches */
  /* this has not been tested with FFT_COLUMN_BASED, may or may not work */
  fft_real *Dgrid_local;
  Dgrid_local = (fft_real *)Mem.mymalloc_movable_clear(&Dgrid_local, "Dgrid_local", maxfftsize * sizeof(fft_real));

  fft_complex *fft_of_Dgrid_local;
  fft_of_Dgrid_local = (fft_complex *)&Dgrid_local[0];
  ...
  /* There is a chance the amount of information is less than maxfftsize, depending on the fftw plan. This will keep a count of how much actual information is accounted for, and to ensure we don't write redundant information in the file. */
  long int Dgrid_counter = 0;
...
#ifdef CB_VELDIV
          double theta_re = - kx * fft_of_velgridx[ip][1] - ky * fft_of_velgridy[ip][1] - kz * fft_of_velgridz[ip][1];
          double theta_im = kx * fft_of_velgridx[ip][0] + ky * fft_of_velgridy[ip][0] + kz * fft_of_velgridz[ip][0];
          double pt = (theta_re * theta_re + theta_im + theta_im);

          double confHub = All.Time * All.HubbleParam * Nulinear.Hubble_Extern(All.Time);

          pt *= smth;
          pt /= confHub * confHub;
#endif

          /* put the 3D vector delta in the local_grid_buf container */
          if(flag == 0) 
            {
              fft_of_Dgrid_local[ip][0] = fac * sqrt(smth) * fft_of_rhogrid[ip][0];
              fft_of_Dgrid_local[ip][1] = fac * sqrt(smth) * fft_of_rhogrid[ip][1];

	      Dgrid_counter += 2; // double count for both the re and im parts
            }
  ...
  for(int i = 0; i < BINS_PS; i++)
    {
      ...
      if(CountModes[i] > 0) {
        PowerUncorrected[i] = SumPowerUncorrected[i] / CountModes[i];
#ifdef CB_VELDIV
        ThetaUncorrected[i] = SumThetaUncorrected[i] / CountModes[i];
#endif
      } else {
        PowerUncorrected[i] = 0;
#ifdef CB_VELDIV
        ThetaUncorrected[i] = 0;
#endif
      }
  ...
  /* a subroutine to write out the rhogrid for phase information, only do it for the CDM sector */
  /* there are a lot of MPI_Barriers, because we need the processes to write in the exact order according to their rank */
  /* rank = 0 process initialise the file, all other ranks append to the file with their local patch */
  MPI_Barrier(Communicator);
  if(flag == 0) 
    {
#ifndef ADDITIONAL_GRID
      if(typeflag[1] == 1 && typeflag[0] == 1)
#else
      if(typeflag[1] == 1 && typeflag[0] == 1)
#endif
        {
          if(ThisTask == 0)
            {
              ...
              FILE *fdgrid;
              if(!(fdgrid = fopen(delta_grid_fname, "w")))
                {
                  Terminate("can't open file '%s'\n", delta_grid_fname);
                }
              ...
              if(ThisTask == nt)
                {
                  ...
                  FILE *fdgrid;
                  if(!(fdgrid = fopen(delta_grid_fname, "a")))
                    {
  ...
  if(flag == 0)
    { 
      mpi_printf("Finished writing rhogrid phase info on all processes\n");
    }

  Mem.myfree(Dgrid_local);

#ifdef CB_VELDIV
  if(ThisTask == 0 && flag == 0)
    {
      char buf[MAXLEN_PATH_EXTRA];
      sprintf(buf, "%s/thetaspecs", All.OutputDir);
      mkdir(buf, 02755);

      sprintf(theta_spec_fname, "%s/thetaspecs/thetaspec_%.3f.txt", All.OutputDir, All.Time);

      FILE *fdt; 
      if(!(fdt = fopen(theta_spec_fname, "w")))
        Terminate("can't open file '%s'\n", theta_spec_fname);
